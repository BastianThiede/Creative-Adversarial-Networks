...
def CAN_loss(model):
    ...
    model.d_loss_real = tf.reduce_mean(
      sigmoid_cross_entropy_with_logits(tf.add(model.D_, tf.constant(1e-15)) ,
                                        true_label * tf.ones_like(model.D)))

    model.d_loss_fake = tf.reduce_mean(
      sigmoid_cross_entropy_with_logits(tf.add(model.D_logits_, tf.constant(1e-15)),
                                        false_label * tf.ones_like(model.D_)))

    model.d_loss_class_real = tf.reduce_mean(
      tf.nn.softmax_cross_entropy_with_logits(logits=tf.add(model.D_c_logits, tf.constant(1e-15)),
    ...
    if model.style_net_checkpoint is None:
      ...
      model.g_loss_class_fake = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=tf.add(model.classifier, tf.constant(1e-15)),
      labels=(1.0/model.y_dim)*tf.ones_like(model.D_c_)))

    model.g_loss_fake = -tf.reduce_mean(tf.log(tf.add(model.D_, tf.constant(1e-15))))
