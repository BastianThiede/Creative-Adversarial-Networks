...
def CAN_loss(model):
    ...
    model.d_loss_real = - tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(model.D_logits,tf.ones_like(model.D)))
    ...
    model.d_loss_fake = - tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(model.D_logits_,tf.zeros_like(model.D_)))
    ...
    model.g_loss_fake = - tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(model.D_logits_,
